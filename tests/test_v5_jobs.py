...
from v5.jobs import produce,freeze,confirm_frozen
...
def test_feature_freeze_persists_independent_pointer_before_confirmation():
    with TemporaryDirectory() as d:
        root=Path(d);universe=UniverseV1.build(trade_date="2026-08-14",created_at=NOW,codes=["000001"],sources=["v5_seed"]);universe.save(root);at=NOW.replace(hour=14,minute=49);signal=snap(at,"signal");result=freeze(root,now=at,sources=(Source("sina",signal),Source("eastmoney",signal)));assert result["snapshot_id"]==signal.snapshot_id and result["acquisition_session_id"].startswith("acq1-") and (root/"frozen/2026-08-14/signal.json").exists()
