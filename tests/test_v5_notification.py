...
def facts(root):
    ...
    (root/"acquisition"/day/"a.json").write_text(json.dumps({"accepted":True,"stage":"morning"}),encoding="utf-8")
    (root/"acquisition"/day/"b.json").write_text(json.dumps({"accepted":True,"stage":"signal"}),encoding="utf-8")
...
def test_failed_gate_never_sends_and_accepted_receipt_is_idempotent():
    with TemporaryDirectory() as d:
        ...
        (root/"acquisition"/day/"z.json").write_text(json.dumps({"accepted":False,"stage":"signal"}),encoding="utf-8")
