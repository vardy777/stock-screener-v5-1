...
def test_buy_sell_t1_fees_idempotency_and_reconciliation():
    with TemporaryDirectory() as directory:
        ...
        try:engine.execute(order("SELL","2026-08-13",buy.shares),at=NOW+timedelta(minutes=1))
        except ContractViolation as exc:assert "strict 09:30" in str(exc)
        else:raise AssertionError("same-day afternoon sell is not strict exit evidence")
        strict_sell_at=datetime(2026,8,14,9,30,10,tzinfo=CHINA_TZ);sell=PaperOrderV1("decision-1","SELL","000001","2026-08-14",strict_sell_at.isoformat(),"10.2",buy.shares,"ms1-sell","2026-08-14")
        filled=engine.execute(sell,at=strict_sell_at);assert filled.outcome=="FILLED";assert ledger.reconcile()["passed"]
...
def test_account_and_round_trips_obey_as_of_cutoff():
    with TemporaryDirectory() as directory:
        ...
        sell_at=datetime(2026,8,14,9,30,10,tzinfo=CHINA_TZ);sell=PaperOrderV1("d","SELL","000001","2026-08-14",sell_at.isoformat(),"10.2",buy.shares,"ms2","2026-08-14");engine.execute(sell,at=sell_at)
