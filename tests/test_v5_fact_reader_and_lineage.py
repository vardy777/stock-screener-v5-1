...
def test_daily_lineage_acceptance_proves_snapshot_decision_and_notification_chain():
    with TemporaryDirectory() as d:
        ...
        write(root/f"morning_pools/{day}/p.json",{"pool_id":pool,"created_at":NOW.isoformat(),"snapshot_id":morning,"candidates":[candidate]})
        write(root/f"confirmations/{day}/c.json",{"confirmation_id":confirmation,"decided_at":NOW.replace(hour=14,minute=50).isoformat(),"morning_pool_id":pool,"snapshot_id":signal,"candidates":[candidate]})
