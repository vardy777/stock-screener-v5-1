...
$rawDays=& $python -X utf8 -c "import csv,sys; rows=list(csv.DictReader(open(sys.argv[1],encoding='utf-8-sig'))); start=sys.argv[2]; n=int(sys.argv[3]); print('\n'.join([r['date'] for r in rows if r['date']>=start and r['is_open']=='1'][:n]))" $calendar $StartDate.ToString("yyyy-MM-dd") $TradingDays
$days=@(($rawDays -join "`n") -split "`r?`n" | Where-Object {$_ -match '^\d{4}-\d{2}-\d{2}$'})
