...
class SinaRealtimeSource:
    ...
    def __init__(self,fetch_text=None,timeout=15,batch_size=700,clock=None):self.fetch_text=fetch_text or _fetch;self.timeout=timeout;self.batch_size=batch_size;self.clock=clock or (lambda:datetime.now(CHINA_TZ))
    def capture(self,codes,*,stage,now):
        started=self.clock();quotes=[]
        for offset in range(0,len(codes),self.batch_size):
            batch=codes[offset:offset+self.batch_size];symbols=[("sh" if x.startswith("6") else "sz")+x for x in batch];text=self.fetch_text("https://hq.sinajs.cn/list="+",".join(symbols),self.timeout);received=self.clock()
            ...
            if offset+self.batch_size<len(codes):time.sleep(.15)
