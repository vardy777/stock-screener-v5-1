...
def evaluate(pairing_facts,protocol=None):
    ...
    kill=bool(invalid) or drawdown>protocol.kill_max_drawdown or (len(rows)>=protocol.early_kill_days and high is not None and high<0)
    return {"schema_version":"v5-challenger-evaluation-v2","protocol_id":protocol.protocol_id,"observed_pairing_facts":len(all_rows),"paired_eligible_days":len(rows),"paired_days":len(rows),"excluded_ineligible_days":len(invalid),"market_regime_counts":market,"regime_counts":market,"turnover_regime_counts":turnover,"large_decline_count":declines,"coverage_passed":coverage,"paired_mean_excess":mean,"paired_excess_ci95":[low,high],"maximum_compounded_drawdown":drawdown,"maximum_drawdown":drawdown,"walk_forward":segments,"decision":"KILL" if kill else "PROMOTE" if promote else "CONTINUE_RESEARCH"}
