...
def run(root,*,now=None,transport=None,sources=None):
    ...
    first,second=sources or (SinaRealtimeSource(),TencentRealtimeSource());result=ConsensusAcquirer(first,second).acquire(universe,stage="signal",now=current)
    ...
    candidates=funnel.to_dict()["candidates"][:5];payload={"schema_version":"v5-recovery-observation-v1","trade_date":day,"observed_at":current.isoformat(),"snapshot_id":result.primary.snapshot_id,"market_state_id":market.market_state_id,"source_consensus":result.report,"candidates":candidates,"strict_0925_sample":False,"eligible_for_confirmation":False,"eligible_for_paper":False,"research_locked":True}
    ...
    if transport is None:
        post=urlencode({"token":_token(root.parent/".env"),"title":f"V5午后恢复观察 {day}","content":content,"template":"html"}).encode()
        transport=lambda:json.loads(urlopen(Request("https://www.pushplus.plus/send",data=post,headers={"Content-Type":"application/x-www-form-urlencoded"}),timeout=15).read().decode())
    response=transport()
