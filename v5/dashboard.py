...
def render(model,view="today"):
    ...
    if view=="today":
        content=f"<section class='hero'><div class='label'>今天的结论</div><h1>{html.escape(today['action'])}</h1><div class='action'><b>当前可执行动作</b><br>{'等待严格数据进入影子采集' if today['data_quality']=='unavailable' else '仅执行系统冻结的本地模拟流程'}</div><p class='muted'>数据时间：{html.escape(str(today.get('data_as_of') or '无'))} · 来源：{html.escape(today.get('source') or '无')} · 快照：{html.escape(today.get('snapshot_id') or '无')}</p></section><div class='grid'><section class='card span4'><div class='muted'>行情质量</div><div class='stat {'good' if today['data_quality']=='accepted' else 'bad'}'>{today['data_quality']}</div></section><section class='card span4'><div class='muted'>全市场覆盖率</div><div class='stat'>{f'{today["coverage"]*100:.1f}%' if today['coverage'] is not None else '无数据'}</div></section><section class='card span4'><div class='muted'>当前候选</div><div class='stat'>{today['candidate_count']}</div></section></div>"
    elif view=="candidates":
        rows="".join(f"<tr><td>#{x['rank']}</td><td>{html.escape(x['name'])}<br><span class='muted'>{x['code']}</span></td><td>{x['change_pct']:.2f}%</td><td>{x['score_percentile']:.0%}</td><td>{'、'.join(x['reasons'])}<br><span class='bad'>风险：{'、'.join(x.get('risks',[]))}</span><br><span class='muted'>依据：{html.escape(x.get('rank_basis',''))}</span></td></tr>" for x in c["items"])
