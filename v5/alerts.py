...
import hashlib,json,os,msvcrt,time
...
def send_failure(root,trade_date,task,error,env_path,*,transport=None):
    ...
    lock_path=root/"alert_locks"/trade_date/task/f"{fingerprint}.lock";lock_path.parent.mkdir(parents=True,exist_ok=True)
    with lock_path.open("a+b") as lock:
        deadline=time.monotonic()+20
        while True:
            try:lock.seek(0);msvcrt.locking(lock.fileno(),msvcrt.LK_NBLCK,1);break
            except OSError:
                if time.monotonic()>=deadline:raise ContractViolation("V5 alert lock timeout")
                time.sleep(.02)
        try:
