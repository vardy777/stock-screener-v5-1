...
def save_opportunity(root,*,trade_date,created_at,morning_pool_id,morning_observed_at,decision_snapshot_id,decision_snapshot_at,confirmation_at,baseline_confirmation_id,challenger_confirmation_id,baseline_code,challenger_code,buy_execution_snapshot_id,buy_execution_at,market_regime,turnover_regime,large_index_decline=False,index_decline_status="UNKNOWN",index_benchmark_id=""):
    ...
    verified_decline=index_decline_status=="VERIFIED_DECLINE" and bool(index_benchmark_id)
    value={"schema_version":"v5-eligible-opportunity-day-v3","trade_date":trade_date,"created_at":created_at,"morning_pool_id":morning_pool_id,"morning_observed_at":morning_observed_at,"decision_snapshot_id":decision_snapshot_id,"decision_snapshot_at":decision_snapshot_at,"confirmation_at":confirmation_at,"baseline_confirmation_id":baseline_confirmation_id,"challenger_confirmation_id":challenger_confirmation_id,"baseline_code":baseline_code,"challenger_code":challenger_code,"buy_execution_snapshot_id":buy_execution_snapshot_id,"buy_execution_at":buy_execution_at,"market_regime":market_regime,"turnover_regime":turnover_regime,"index_decline_status":index_decline_status,"index_benchmark_id":index_benchmark_id,"large_index_decline":verified_decline,"eligible":eligible}
...
def _validate_references(root,row):
    ...
    index=resolve_index_benchmark(root,day,as_of=row["created_at"])
    if row.get("index_decline_status","UNKNOWN")!=index["status"] or row.get("index_benchmark_id","")!=index.get("index_benchmark_id",""):raise ContractViolation("opportunity index benchmark lineage mismatch")
    return True
