...
def health(root,day,now):
    ...
    checks={"universe_exists":_latest(root,"universes",day) is not None,"morning_fact_exists":morning is not None,"confirmation_fact_exists":confirmation is not None,"morning_notification_accepted":False,"confirmation_notification_accepted":False,"paper_ledger_reconciled":PaperLedger(root/"paper").reconcile()["passed"]}
    ...
    recovery=ShadowScheduler(root).recovery_report(day,now);report={"schema_version":"v5-health-v1","trade_date":day,"recorded_at":now.astimezone(CHINA_TZ).isoformat(),"checks":checks,"recovery":recovery,"passed":all(checks.values()) and recovery["status"]=="CLEAN"};return report
