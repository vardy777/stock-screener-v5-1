...
def replay(*,universe,morning_sources,confirmation_sources,morning_at,confirmation_at,sell_snapshot,sell_at,ledger):
    ...
    cf=funnel.run(confirm.primary,market_state_id="mstate1-replay-confirm",market_valid=True,stage="confirmation",allowed_codes=[x["code"] for x in pool.candidates],baseline_candidates=pool.candidates);decision=ConfirmationV5.from_funnel(pool,cf,decided_at=confirmation_at)
