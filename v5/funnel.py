...
class CandidateFunnel:
    ...
    def run(self,snapshot,*,market_state_id:str,market_valid:bool,stage:str,allowed_codes:Iterable[str]|None=None,baseline_candidates:Iterable[dict]|None=None)->CandidateFunnelV1:
        ...
        tradeable=[];rejected={}
        completed=datetime.fromisoformat(snapshot.batch_completed_at)
        for q in raw:
            quote_age=(completed-datetime.fromisoformat(q.exchange_time)).total_seconds();upper_name=q.name.upper();reason="special_treatment" if "ST" in upper_name or "退" in q.name else "halted" if q.halted else "stale_symbol_quote" if quote_age<0 or quote_age>self.policy.maximum_symbol_quote_age_seconds else "limit_locked" if q.limit_up or q.limit_down else "missing_buy_book" if stage=="confirmation" and (q.ask1<=0 or q.ask1_volume<=0) else ""
