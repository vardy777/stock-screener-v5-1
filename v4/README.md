...
看板以“验证V4是否有效”而非展示候选数量为第一目标，并严格分开三类证据：

- V4严格前向样本：真实14:50决策与下一交易日09:30成交的配对记录，是唯一
  可以决定模型准入的主证据，最低要求500笔并显示胜率95%区间；
- 旧版V3模拟账户：保留原交易记录、资金曲线、回撤和逐笔盈亏，只作为迁移
  旁证，不并入V4绩效；
- 15:00代理Walk-Forward：明确标为仅研究，不能与严格样本或模拟账户合并。

`?mode=chase`继续兼容原地址，但现在表示“自动总览”视图，不再人工指定执行
策略。系统根据新鲜全市场覆盖、市场宽度、等权涨跌、开盘缺口和情绪分选择
追高观察、回调观察、双池精选或空仓；该适配层只排列旧版研究池，不能绕过
V4模型、Top1、交易时钟、风险和研究门禁。

市场模块不再把全市场等权涨跌误标为上证指数。看板同时展示行情时间、快照
覆盖率、实时覆盖率、上涨/下跌家数、样本成交额及金额覆盖。历史主力资金缓存
超过3天会自动降级且不参与当日信号。现有板块分类只是名称关键词代理；覆盖
不足60%时，板块结果只展示、不参与候选加减分，等待后续接入逐证券时点行业
映射后再启用。
