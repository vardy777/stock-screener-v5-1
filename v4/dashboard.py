...
def _build_candidates_html(
    ...
    for candidate in visible:
        ...
        tradable = bool(candidate.get('v4_paper_eligible'))
        ...
        linkage = (
            f'早盘#{_safe(candidate.get("morning_rank","?"))} → '
            f'尾盘#{_safe(candidate.get("rank","?"))}'
            if candidate.get('morning_pool_member') else '待尾盘母池确认'
        )
        ...
        <div class="candidate-card {'fit' if fit else 'blocked'}" data-strategy="{_safe(strategy)}">
          ...
          <div class="candidate-decision"><span class="tag {decision_class}">{'允许模拟观测' if tradable else '观察/空仓'}</span><div style="margin-top:7px;color:#9fb4cc;font-size:11px">{_safe(strategy_label)}</div><div class="shadow">{_safe(linkage)}</div><div class="shadow">V4研究排序强度 {shadow_score:.0f}/100 · 非概率</div><div class="shadow">来源 {_safe(candidate.get('candidate_source','未证明'))}</div></div>
        </div>""")
    ...
    <div style="margin-top:12px;display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap">
      <span style="color:#64748b;font-size:11px">仅V4早盘母池中经14:50确认的Top1可进入研究模拟；生产门禁仍独立锁定。</span>
      <button class="btn btn-buy-sm" onclick="buySelected()" {buy_disabled}>模拟买入已确认Top1</button>
    </div>"""
...
def run_dashboard(port: int = None):
    ...
    from v4.paper_scheduler import start_paper_scheduler
