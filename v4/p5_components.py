...
<section class='card span6' data-component='cutover-readiness'><h2>生产切换准备</h2><p>准备状态：<b>{'READY' if cutover.get('ready') else 'BLOCKED'}</b></p><p>授权：<b>{'允许' if cutover.get('apply_allowed') else '未授权'}</b></p><p class='muted'>{_esc(cutover.get('plan_id','尚无切换计划'))}</p></section>"""
