...
class DataFetcher:
    ...
    def _parse_sina_quotes(self, text: str) -> Optional[pd.DataFrame]:
        ...
        for line in text.strip().split('\n'):
            ...
            try:
                ...
                volume_hand = float(fields[8]) if fields[8] else 0.0
    ...
    def fetch_kline(self, code: str, days: int = 30,
        ...
        days : int
            获取天数。
        scale : int
            周期分钟数，60=日线。传 240 或 'daily' 也视为日线。
        ...
        for r in records:
            try:
                ...
                volume = float(r.get('volume', 0))  # 手, 转换为股
    ...
    @staticmethod
    def get_market_state(sh_code: str = "sh000001",
                         sample_codes: Optional[list] = None) -> dict:
        """
        获取多周期市场状态（替代单日涨跌判断）。
        ...
        dict
            ...
            volume_ratio: 今日量比 vs 5日均量
            composite: 综合评分 (-4~+4.5)
            mode_label: risk_on / risk_off / neutral
        """
        result = {"sh_1d_pct": 0, "sh_5d_pct": 0, "sh_20d_pct": 0,
                  "advance_ratio": 0.5, "volume_ratio": 1.0,
                  "composite": 0, "mode_label": "neutral"}

        df = DataFetcher()  # 实例化以调用实例方法
        ...
        # 2) 多周期K线数据
        kline = df.fetch_kline(sh_code, days=25)
        if kline is not None and len(kline) >= 5:
            closes = kline["close"].values
            today_close = closes[-1]
            result["sh_5d_pct"] = round((today_close / closes[-5] - 1) * 100, 2)
            if len(closes) >= 20:
                result["sh_20d_pct"] = round((today_close / closes[-20] - 1) * 100, 2)

        # 3) 市场宽度 (上涨家数占比)
        if sample_codes:
            quotes = df.batch_fetch_quotes(sample_codes)
            if quotes is not None and not quotes.empty and "pct_chg" in quotes.columns:
                up = (quotes["pct_chg"] > 0).sum()
                total = len(quotes)
                result["advance_ratio"] = round(up / total, 3) if total > 0 else 0.5
        ...
        # 5日: ±1分 (阈值±2%)
        sh_5d = result["sh_5d_pct"]
        score += 1 if sh_5d > 2 else (-1 if sh_5d < -2 else 0)
        # 20日: ±1分 (阈值±4%)
        sh_20d = result["sh_20d_pct"]
        score += 1 if sh_20d > 4 else (-1 if sh_20d < -4 else 0)
        # 宽度: ±1分 (>60%上涨=+1, <40%=-1)
        ar = result["advance_ratio"]
        score += 1 if ar > 0.6 else (-1 if ar < 0.4 else 0)
        # 量能: ±0.5分 (量比>1.2=+0.5, <0.8=-0.5)
        vr = result.get("volume_ratio", 1.0)
        score += 0.5 if vr > 1.2 else (-0.5 if vr < 0.8 else 0)

        result["composite"] = round(score, 1)
        if score >= 1.5:
            result["mode_label"] = "risk_on"
        elif score <= -1.5:
            result["mode_label"] = "risk_off"
        else:
            result["mode_label"] = "neutral"
