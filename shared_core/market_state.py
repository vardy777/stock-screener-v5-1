...
from .core import ContractViolation,strict_bool,strict_int,strict_number
...
class MarketStateV1:
    ...
    def from_mapping(cls,value):
        required={"trade_date","snapshot_id","total","advancers","decliners","unchanged","limit_up","limit_down","halted","total_amount","median_change","advance_ratio","severe_decline_ratio","regime","trade_allowed","reasons","policy_version","schema_version","market_state_id"}
        if set(value)!=required:raise ContractViolation("market state persisted keys invalid")
        if type(value["reasons"]) is not list or not all(type(x) is str for x in value["reasons"]):raise ContractViolation("market state reasons invalid")
        state=cls(value["trade_date"],value["snapshot_id"],strict_int(value["total"],"total",0),strict_int(value["advancers"],"advancers",0),strict_int(value["decliners"],"decliners",0),strict_int(value["unchanged"],"unchanged",0),strict_int(value["limit_up"],"limit_up",0),strict_int(value["limit_down"],"limit_down",0),strict_int(value["halted"],"halted",0),strict_number(value["total_amount"],"total_amount"),strict_number(value["median_change"],"median_change"),strict_number(value["advance_ratio"],"advance_ratio"),strict_number(value["severe_decline_ratio"],"severe_decline_ratio"),value["regime"],strict_bool(value["trade_allowed"],"trade_allowed"),tuple(value["reasons"]),value["policy_version"],value["schema_version"])
