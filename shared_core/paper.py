...
import msvcrt,time
from .core import ContractViolation,strict_int
...
class PaperOrderV1:
    ...
    def __post_init__(self):
        strict_int(self.shares,"shares",0)
        if self.side not in {"BUY","SELL"} or len(self.code)!=6 or not self.code.isdigit() or type(self.reference_price) is not str:raise ContractViolation("paper order contract invalid")
    @property
...
class PaperEventV1:
    ...
    def __post_init__(self):
        strict_int(self.shares,"shares",0)
        if self.outcome not in {"FILLED","REJECTED"} or self.side not in {"BUY","SELL"} or len(self.code)!=6 or not self.code.isdigit():raise ContractViolation("paper event contract invalid")
        if not all(type(x) is str for x in (self.fill_price,self.commission,self.tax,self.cash_flow)):raise ContractViolation("paper event monetary strings required")
    @property
...
class PaperLedger:
    ...
    def events(self):
        ...
        if set(data)!={"schema_version","initial_cash","head","events"} or data.get("schema_version")!="v5-paper-ledger-v1" or type(rows) is not list:raise ContractViolation("paper ledger persisted contract invalid")
        previous="genesis"
        for index,row in enumerate(rows,1):
            if set(row)!={"sequence","previous_event_id","event_id","event"} or type(row.get("event")) is not dict:raise ContractViolation("paper event envelope invalid")
    ...
    def pending_orders(self):
        ...
        for path in (self.root/"orders").glob("*/*.json") if (self.root/"orders").exists() else []:
            ...
            if set(row)!=set(PaperOrderV1.__dataclass_fields__):raise ContractViolation("paper order persisted keys invalid")
