...
class ConsensusResult:
    accepted:bool;primary:object|None;report:dict;sources:tuple=()
...
class ConsensusAcquirer:
    ...
    def acquire(self,universe:UniverseV1,*,stage:str,now:datetime)->ConsensusResult:
        ...
        if None in snapshots:return ConsensusResult(False,None,report,tuple(x for x in snapshots if x is not None))
        ...
        for code in common:
            ...
            elif stage in {"sell","sell_execution"}:
        ...
        return ConsensusResult(accepted,primary if accepted else None,report,tuple(snapshots))
