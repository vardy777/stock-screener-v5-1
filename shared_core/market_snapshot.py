...
from .core import CHINA_TZ,ContractViolation,strict_bool,strict_int,strict_number
...
class QuoteV1:
    ...
    def from_mapping(cls,row:Mapping[str,Any]):
        ...
        return cls(code,name,declared.isoformat(),exchange.isoformat(),provider_time.isoformat(),received.isoformat(),
            ...
            _integer(row.get("volume"),"volume"),_number(row.get("amount"),"amount"),_boolean(row.get("halted"),"halted"),_boolean(row.get("limit_up"),"limit_up"),_boolean(row.get("limit_down"),"limit_down"),provider)
...
class MarketSnapshotV1:
    ...
    def build(cls,*,trade_date,session,batch_started_at,batch_completed_at,quotes:Iterable[QuoteV1],expected_codes,minimum_coverage=.95,maximum_age_seconds=30,maximum_batch_seconds=30):
        started=_aware(batch_started_at,"batch_started_at");completed=_aware(batch_completed_at,"batch_completed_at");items=tuple(quotes);expected=strict_int(expected_codes,"expected_codes",1)
